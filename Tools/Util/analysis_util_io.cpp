...
namespace analysis_util {
  ...
  int find_var_index(const Vector<std::string>& var_names,
                     ...
                     bool abort_if_not_found)
  {
    ...
    if (abort_if_not_found) {
      amrex::Abort("analysis_util::find_var_index: variable '" + name +
  ...
  static BoxArray ensure_min_boxes(const BoxArray& ba, int nprocs)
  {
    if (static_cast<int>(ba.size()) >= nprocs) return ba;
    const int ncells_per_rank = std::max(1, static_cast<int>(ba.numPts()) / nprocs);
    const int max_size = std::max(1,
        static_cast<int>(std::pow(static_cast<double>(ncells_per_rank),
                                  1.0 / AMREX_SPACEDIM)));
  ...
  PlotfileData read_plotfile(const std::string& infile,
                             ...
                             const Vector<int>& is_per_in)
  {
    ...
    if (!dataServices.AmrDataOk()) {
      amrex::Abort("analysis_util::read_plotfile: cannot open plotfile: " + infile);
    }
    ...
    for (int i = 0; i < ncomp; ++i) dest_comps[i] = i;
    ...
    for (int lev = 0; lev < n_lev; ++lev) {
      ...
      result.geoms[lev] = Geometry(amrData.ProbDomain()[lev], &rb, coord, is_per.data());
  ...
  void write_plotfile(const std::string& outfile,
                      ...
                      const Vector<int>& ref_ratios_in)
  {
    ...
    if (needs_split) {
      ...
      WriteMultiLevelPlotfile(outfile, n_lev, GetVecOfConstPtrs(mf_split),
                              var_names, geoms, time, isteps, ref_ratios);
    } else {
      WriteMultiLevelPlotfile(outfile, n_lev, GetVecOfConstPtrs(mf),
                              var_names, geoms, time, isteps, ref_ratios);
    }
