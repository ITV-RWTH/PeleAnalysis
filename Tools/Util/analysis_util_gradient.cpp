...
namespace analysis_util {
  ...
  void gradient(const amrex::Vector<amrex::MultiFab> a_mf, const int scomp, const int ncomp, amrex::Vector<amrex::MultiFab> grad_mf) {
	...
	for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
	  ...
	  grad[lev][idim].define(amrex::convert(ba,amrex::IntVect::TheDimensionVector(idim)),
				 dmap[lev], 1, nGrowGrad);
	}    
	phi[lev] = std::make_unique<amrex::MultiFab> (a_mf[lev],amrex::make_alias,scomp+n,1);
	...
	lap.emplace_back(grids[lev], dmap[lev], 1, 1);
      ...
      mlmg.getFluxes(GetVecOfArrOfPtrs(grad), GetVecOfPtrs(phi), amrex::MLMG::Location::FaceCenter);
      for (int lev = 0; lev < nlev; ++lev) {
	// Convert to cell avg gradient
	amrex::MultiFab gradAlias(grad_mf[lev], amrex::make_alias, n*AMREX_SPACEDIM, AMREX_SPACEDIM);
