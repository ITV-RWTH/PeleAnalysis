...
namespace analysis_util {
  ...
  MEFData read_mef(const std::string& infile)
  {
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(ifs.good(),
      "analysis_util::read_mef: cannot open file " + infile);
    ...
    ifs.read(reinterpret_cast<char*>(data.connectivity.data()),
             sizeof(int) * n_conn);
  ...
  void write_mef(const std::string& outfile, const MEFData& data)
  {
    if (amrex::ParallelDescriptor::IOProcessor()) {
      ...
      AMREX_ALWAYS_ASSERT_WITH_MESSAGE(ofs.good(),
        "analysis_util::write_mef: cannot open file " + outfile);
      ...
      ofs.write(reinterpret_cast<const char*>(data.connectivity.data()),
