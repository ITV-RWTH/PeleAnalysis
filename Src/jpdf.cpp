...
static std::string ProtectSlashes(const std::string str) {
  std::string s = str;
...
static void print_usage(int, char *argv[]) {
  ...
  std::cerr << argv[0] << " infile=<filename>\n"
            ...
            << "   output_tecplot=[0/1]    : Output JPDFs for tecplot          "
               "   (default=0)\n"
            << "   output_scatter=[0/1]    : Output scatter plot of non-zero "
               "bins (default=0)\n"
            << "   output_fab=[0/1]        : Output JPDFs in fab format        "
               "   (default=0)\n"
            << "   output_plotfile=[0/1]   : Output JPDFs in plotfile format   "
               "   (default=1)\n"
            << "   vars= var1 var2 var3    : Variable list; JPDF will be "
               "evaluated for each pair\n"
            << "   useminmax%i= min max    : Use min/max for var %i\n"
            << "   condMean_vars = var     : Variable that will be averaged "
               "conditioned on vars\n"
            << "   nBins=n                 : Number of bins in each direction "
               "for JPDF (default=64)\n"
            << "   finestLevel=n           : Finest level at which to evaluate "
               "JPDFs\n"
            << "   outSuffix=str           : Suffix to add to the plfile name "
               "as an alt dir for results (default="
               ")\n"
            << "   do_conditioning=[0/1/2] : 0 -> no conditioning (default)\n"
            << "                             1 -> conditioning on a loaded "
               "variable \n"
            << "                             2 ->  Evaluation with c(1-c)\n"
            << "     cVar=[0...nVars]      : Index in 'vars' of var for "
               "conditioning (default=0)\n"
            << "     cMin = min            : Min value for conditioning "
               "(default=0.0)\n"
            << "     cMax = max            : Max value for conditioning "
               "(default=1.0)\n"
            << "     norm_cVal=[0/1]       : Normalize cVar (default=0, forced "
               "for do_conditioning=2)\n"
            << "        cNormMin = min     : Min value for normalization "
               "(default=0.0)\n"
            << "        cNormMax = max     : Max value for normalization "
               "(default=1.0)\n"
            << "   do_average=[0/1]        : Performs an average over all "
               "provided plot files (default=0)\n"
    ...
    for (int iPlot = 0; iPlot < nPlotFiles; iPlot++) {
      ...
      for (int var1 = 0; var1 < nVars_jpdf; var1++) {
            ...
            for (int var2 = var1 + 1; var2 < nVars_jpdf; var2++) {
              ...
              for (int v1i = 0, i = 0; v1i < nBins; v1i++) {
                ...
                for (int v2i = 0; v2i < nBins; v2i++, i++) {
                  ...
                  } else { // This is the bit that breaks the derivation of the
                           // 1D pdfs
                    binX1[i] = v1;
                    binX2[i] = v2;
                  }
              ...
              // second) Is this correct, if conditioning is applied? In that
              // case sum(bin)!=domainVol
              ...
              if (output_matlab) {
                ...
                for (int v = 0; v < nVars_condMean; v++) {
                  ...
                  filename = infile + outSuffix + "/condMean_" +
                             ...
                             whichVarOut[var1] + "_" + whichVarOut[var2] +
                             ".dat";
                  std::cout << "Opening file " << filename << std::endl;
                  file = fopen(filename.c_str(), "w");
                  for (int v1i = 0; v1i < nBins; v1i++) {
              ...
              if (output_tecplot) {
                ...
                // Format: Header, each node, quadrilateral indices between
                // nodes
                filename = infile + outSuffix + "/Pdf_" + whichVarOut[var1] +
              ...
              if (output_scatter) {
                ...
                // Format: x y
                filename = infile + outSuffix + "/Scatter_" +
        ...
        if (output_plotfile) {
          ...
          BoxArray ba(Box(IntVect::TheZeroVector(),
                          IntVect(AMREX_D_DECL(nBins - 1, nBins - 1, 0))));
