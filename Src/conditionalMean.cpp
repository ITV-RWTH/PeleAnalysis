...
print_usage(int, char* argv[])
{
  std::cerr << "Usage:\n"
            ...
            << "  finestLevel=N      Finest AMR level to read (default: finest in plotfile)\n"
            << "  outSuffix=STRING   Suffix appended to output file names (default: none)\n"
            << "  bounds=xlo ylo zlo xhi yhi zhi   Restrict to a sub-domain (default: full domain)\n"
            << "  writeBinMinMax=0/1 Also write per-bin min/max values (default: 0)\n"
            << "  aja=0/1            Write header to a separate .key file for gnuplot/matlab (default: 0)\n"
