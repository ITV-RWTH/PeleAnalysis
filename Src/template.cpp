...
print_usage(int, char* argv[])
{
  std::cerr << "Usage:\n"
            ...
            << "  vars=\"...\"         Space-separated list of variable names to process\n\n"

            << "Options:\n"
            << "  finestLevel=N      Cap AMR levels (default: all levels in file)\n"
            << "  is_per=\"1 1 1\"     Periodicity flags per dimension (default: all 1)\n"
...
main(int argc, char* argv[])
{
  Initialize(argc, argv);
  {
    ...
    if (nvars == 0) {
      amrex::Abort("template: 'vars' is required — specify the variable names to process.\n"
    ...
    // Read plotfile
    // -------------------------------------------------------------------------
    auto data = analysis_util::read_plotfile(infile, vars, finestLevel,
                                             /*n_grow=*/0, is_per);
