/*
 * Work in Progress
 * Will replace jpdf.cpp when finished!
 */
             ...
             char* argv[])
{
    ...
    std::cerr << argv[0] << " infile=<filename>\n"
              ...
              << "   use_ebWeighing= [0/1]: Considers the volume Fraction of the cell when using EB (default 0)\n"
      ...
      char* argv[])
{
    ...
    // Number of bins in each direction
    int use_ebWeighting = 0;
    pp.query("use_ebWeighting",use_ebWeighting);
    if(use_ebWeighting) Print() << "Using Cell volume Fractions to weight the JPdf!" << std::endl; 
    ...
    for (int iPlot=0; iPlot<nPlotFiles; iPlot++) {
        ...
        for (int iVar=0; iVar<lVars; iVar++) {
            ...
            //Print() << "vMin for " << whichVar[iVar] << " is " << vMin[iVar] << std::endl; 
        ...
        Vector<std::unique_ptr<MultiFab>> volFracData(nLevels);
        for (int iLevel=0; iLevel<nLevels; iLevel++) {
            if (verbose)
            ...
            volFracData[iLevel].reset(new MultiFab(amrData.boxArray(iLevel), dm, 1, ngrow));
            ...
            // Load data for EB handling 
            #ifdef AMREX_USE_EB
                amrData.FillVar(*volFracData[iLevel], iLevel, {"volFrac"}, ngrow);
            #else
                volFracData[iLevel]->setVal(1.0);
            #endif
        }
        if (verbose)
            std::cout << "      ...done." << std::endl;
        ...
        // Finding min and max after reading data 
        #ifdef AMREX_USE_EB
            Vector<MultiFab> minmax(nLevels);
            ...
            for (int iVar=0; iVar<lVars; iVar++) {
                ...
                if (nMinMax==0) {
                    ...
                    Print() << "..for " << whichVar[iVar] << std::endl;
                        Real global_min = 1e100;
                        Real global_max = -1e100;
                    for (int iLevel=0; iLevel<nLevels; iLevel++) {
                ...
                #pragma omp parallel if (Gpu::notInLaunchRegion())
                        ...
                        Print() << "Starting MFIter for Level " << iLevel << std::endl;
                        for (MFIter mfi(*volFracData[iLevel],TilingIfNotGPU()); mfi.isValid(); ++mfi)
                        {    
                            const Box& bx = mfi.tilebox();
                            auto const& volFracBox   = volFracData[iLevel]->array(mfi);
                            auto const& varBox  = mf[iLevel]->array(mfi,iVar);
                            //auto const& minmaxBox  = minmax[iLevel].array(mfi,iVar);
                            Real local_min = 1e100;
                            Real local_max = -1e100;


                            AMREX_PARALLEL_FOR_3D(bx, i, j, k, {   
                                
                                Real volFrac = volFracBox(i,j,k); 
                        ...
                        // // Reduction of min and max across all ranks
                        // amrex::ParallelDescriptor::ReduceRealMin(vMin[iVar]);
                        // amrex::ParallelDescriptor::ReduceRealMax(vMax[iVar]);
                        ParallelDescriptor::Barrier();

                        vMin[iVar] = global_min;
                        vMax[iVar] = global_max;
                        Print() << "vMin for " << whichVar[iVar] << " is " << vMin[iVar] << std::endl; 
                        Print() << "vMax for " << whichVar[iVar] << " is " << vMax[iVar] << std::endl; 
                } //if vMin[iVar] == 0 & vMax[iVar] ==  0
            }
        #endif
                        ...
                        for (int k=0; k<kx; k++) {
                            ...
                            for (int j=0; j<jx; j++) {
                                ...
                                for (int i=0; i<ix; i++) {
                                    ...
                                    Real volFrac = vfPtr[cell];
                                    if(volFrac > 1) Print() << "volFrac = " << volFrac << std::endl; 
                                    ...
                                    if (isPtr[cell]>0) {  // Intersect
                                        ...
                                        // Filling of the bins
                                        if (contribute==1) {
                                            int v1i = (int)(nBins*(v1Ptr[cell]-vMin[var1])/(vMax[var1]-vMin[var1]));    // 128 for V1Ptr = vMax and 0 for V1Ptr = vMin
                                            if (v1i<0)      { v1l++; v1i=0; }
                                            if (v1i>=nBins) { v1g++; v1i=nBins-1; } // Necessary because of 0 index system
                                            int v2i = (int)(nBins*(v2Ptr[cell]-vMin[var2])/(vMax[var2]-vMin[var2]));
                                            if (v2i<0)      { v2l++; v2i=0; }
                                            if (v2i>=nBins) { v2g++; v2i=nBins-1; }
                                            if(use_ebWeighting) {
                                                bin[v1i*nBins+v2i]+=Vol*volFrac; // Contains jpdf
                                            } else {
                                                if (volFrac > 0 ) bin[v1i*nBins+v2i]+=Vol;   
                                            }              
                                            binX1[v1i*nBins+v2i]+=Vol*volFrac*v1Ptr[cell];  // Contains volume weighted var1 for that bin
                                            binX2[v1i*nBins+v2i]+=Vol*volFrac*v2Ptr[cell];  // Contains volume weighted var2 for that bin
        ...
        if (ParallelDescriptor::IOProcessor()) {
            ...
            domainVol = 0;
                for (int v1i=0, i=0; v1i<nBins; v1i++) {
                    for (int v2i=0; v2i<nBins; v2i++, i++) {
                        Real *bin=binArray[0].dataPtr();
                        domainVol += bin[v1i*nBins+v2i];
                    }
                }
            ...
            #endif
            Print() << "Domain Volume = " << domainVol << std::endl; 
            ...
            for (int var1=0; var1<nVars; var1++) {
                ...
                for (int var2 = var1+1; var2<nVars; var2++) {
                    ...
                    for (int i=0; i<nBins*nBins; i++)
                        bin[i]/=domainVol;      // Bin contains now respective fraction of domainVol
                    ...
                    if (output_matlab) {
                        ...
                        pp.get("outpath",outpath);
                        outpath = outpath +"/" + "Pdfs/";

                        if (!UtilCreateDirectory(outpath,0755))
                            CreateDirectoryFailed(outpath);

                        std::string prefix = use_ebWeighting ? "Pdf_weighted_" : "Pdf_";
                        filename = prefix + whichVarOut[var1] + "_" + whichVarOut[var2] + ".dat";
                        std::string filepath = outpath + filename;

                        std::cout << "Opening file " << filepath << std::endl;
                        file = fopen(filepath.c_str(),"w");
                        for (int v1i=0; v1i<nBins; v1i++) { // Are the rows 
                            for (int v2i=0; v2i<nBins; v2i++)   // Are the columns
                        ...
                        filename = "Pdf_" + whichVarOut[var1] + "_x.dat";
                        ...
                        filename = "Pdf_" + whichVarOut[var2] + "_x.dat";
                        filepath = outpath + filename;
                        std::cout << "Opening file " << filename << std::endl;
                        file = fopen(filepath.c_str(),"w");
                        for (int v2i=0; v2i<nBins; v2i++)
                        ...
                        filename = "PdfX1_" + whichVarOut[var1] + "_" + whichVarOut[var2] + ".dat";
                        filepath = outpath + filename;
                        std::cout << "Opening file " << filename << std::endl;
                        file = fopen(filepath.c_str(),"w");
                        for (int v1i=0; v1i<nBins; v1i++) {
                            for (int v2i=0; v2i<nBins; v2i++)
                                fprintf(file,"%e ",binX1[v1i*nBins+v2i]);       //v1i is colum and v2i is row
                            fprintf(file,"\n");
                        }
                        fclose(file);
                        ...
                        filename = "PdfX2_" + whichVarOut[var1] + "_" + whichVarOut[var2] + ".dat";
                        filepath = outpath + filename;
                        std::cout << "Opening file " << filename << std::endl;
                        file = fopen(filepath.c_str(),"w");
                        for (int v1i=0; v1i<nBins; v1i++) {
