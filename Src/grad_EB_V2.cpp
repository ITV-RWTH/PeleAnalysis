      ...
      char* argv[])
{
  amrex::Initialize(argc,argv);
  {
    ...
    if( ! dataServices.AmrDataOk()) {
      Print() << "Provided data is not okay!" << std::endl;
    ...
    // Auxiliary variables
    if (verbose > 0) Print() << "Checking for auxiliary variables!" << std::endl;
    ...
    for (int lev=0; lev<Nlev; ++lev) {
      ...
      if (verbose > 0) Print() << "Reading data for level: " << lev << std::endl;
...
// Solver Section
    ...
    if (verbose > 0) Print() << "Setting Boundary Conditions" << std::endl;
    ...
    if (verbose > 0) Print() << "Getting Fluxes!" << std::endl;
