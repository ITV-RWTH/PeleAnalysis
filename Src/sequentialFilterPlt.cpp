...
main(int argc, char** argv)
{
    ...
    AMREX_ALWAYS_ASSERT(les_filter_fgr % 2 == 0);
    ...
    amrex::Vector<amrex::MultiFab> mf1(Nlev);
    amrex::Vector<amrex::MultiFab> mf2(Nlev);
    ...
    for (int lev=0; lev<Nlev; ++lev)
    {
      ...
      weights[lev][0] = 0.5/(amrex::Real)les_filter_fgr_lev;
      weights[lev][les_filter_fgr_lev] = 0.5/(amrex::Real)les_filter_fgr_lev;
      for (int nw = 1; nw < les_filter_fgr_lev; ++nw) {
	weights[lev][nw] = 1.0/(amrex::Real)les_filter_fgr_lev;
      }
    ...
    amrex::Print() << "FillPatching mf1..." << std::endl;
    ...
    for (int lev=0; lev<Nlev; ++lev) {
      ...
      if (lev == 0) {
        ...
        amrex::FillPatchSingleLevel(mf1[lev], mf1[lev].nGrowVect(), 0.0,
                             {&(mf1[lev])},
        ...
        amrex::PhysBCFunct<amrex::GpuBndryFuncFab<pele::physics::pltfilemanager::FillExtDirDummy>> fine_bndry_func(
														   level_geometries[lev], {dummyBCRec}, pele::physics::pltfilemanager::FillExtDirDummy{});
        FillPatchTwoLevels(mf1[lev], mf1[lev].nGrowVect(), 0.0,
                           {&(mf1[lev-1])},
                           {0.0},
                           {&(mf1[lev])},
                           {0.0},
    ...
    amrex::Print() << "Integrating in x..." << std::endl;
    for (int lev=0; lev<Nlev; ++lev)
    {
      int nweights = weights[lev].size();
      int ng =  (nweights - 1)/2;
      amrex::Gpu::DeviceVector<amrex::Real> d_weights(nweights);
      amrex::Real* w = d_weights.data();
      amrex::Gpu::copy(amrex::Gpu::hostToDevice, weights[lev].begin(), weights[lev].end(),d_weights.begin());
      
      amrex::Print() << "on level " << lev << std::endl;

      for (amrex::MFIter mfi(mf1[lev], amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi) {
	const amrex::Box& bx = mfi.validbox();  // Only the valid region (no ghost cells)

	amrex::Array4<const amrex::Real> in_arr = mf1[lev].const_array(mfi);
	amrex::Array4<amrex::Real> intx_arr = mf2[lev].array(mfi);
	
	amrex::ParallelFor(bx,[=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
	  for (int n = 0; n < ncomp_filter; ++n) {
	    amrex::Real sum = 0.0;	  
	    for (int ii = -ng; ii <= ng; ++ii) {
	      sum += w[ng+ii]*in_arr(i+ii,j,k,n);
	    }
	    intx_arr(i,j,k,n) = sum;
	  }
	});
    ...
    amrex::Print() << "Done!" << std::endl;
    amrex::Print() << "Redefining mf1.." << std::endl;
    for (int lev = 0; lev < Nlev; ++lev) {
      const amrex::BoxArray ba = mf1[lev].boxArray();
      const amrex::DistributionMapping dm = mf1[lev].DistributionMap();
      const int nGrow = mf1[lev].nGrowVect()[0];
      const amrex::IntVect nGrow3 = {AMREX_D_DECL(0,0,nGrow)};
      mf1[lev].define(ba,dm,ncomp_filter,nGrow3);
    }
    amrex::Print() << "FillPatching mf2 before y integration..." << std::endl;
    for (int lev=0; lev<Nlev; ++lev) {
      amrex::Print() << "on level "<< lev << std::endl;
      if (lev == 0) {
        amrex::PhysBCFunct<amrex::GpuBndryFuncFab<pele::physics::pltfilemanager::FillExtDirDummy>> bndry_func(
        level_geometries[lev], {dummyBCRec}, pele::physics::pltfilemanager::FillExtDirDummy{});
        amrex::FillPatchSingleLevel(mf2[lev], mf2[lev].nGrowVect(), 0.0,
                             {&(mf2[lev])},
                             {0.0}, 0, 0, ncomp_filter, level_geometries[lev], bndry_func, 0);
      } else {
        amrex::InterpBase* mapper;
        ...
        amrex::PhysBCFunct<amrex::GpuBndryFuncFab<pele::physics::pltfilemanager::FillExtDirDummy>> fine_bndry_func(
          level_geometries[lev], {dummyBCRec}, pele::physics::pltfilemanager::FillExtDirDummy{});
        FillPatchTwoLevels(mf2[lev], mf2[lev].nGrowVect(), 0.0,
                           {&(mf2[lev-1])},
                           {0.0},
                           {&(mf2[lev])},
                           {0.0},
    ...
    amrex::Print() << "Integrating in y..." << std::endl;
    for (int lev=0; lev<Nlev; ++lev)
    {
      int nweights = weights[lev].size();
      int ng =  (nweights - 1)/2;
      ...
      amrex::Gpu::copy(amrex::Gpu::hostToDevice, weights[lev].begin(), weights[lev].end(),d_weights.begin());
      amrex::Print() << "on level " << lev << std::endl;
      for (amrex::MFIter mfi(mf2[lev], amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi) {
	const amrex::Box& bx = mfi.validbox();  // Only the valid region (no ghost cells)
	amrex::Array4<const amrex::Real> intx_arr = mf2[lev].const_array(mfi);
	amrex::Array4<amrex::Real> inty_arr = mf1[lev].array(mfi);
	
	amrex::ParallelFor(bx,[=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
	  for (int n = 0; n < ncomp_filter; ++n) {
	    amrex::Real sum = 0.0;	  
	    for (int jj = -ng; jj <= ng; ++jj) {
	      sum += w[ng+jj]*intx_arr(i,j+jj,k,n);
	    }
	    inty_arr(i,j,k,n) = sum;
	  }
	});
	amrex::Gpu::synchronize();
      }
    ...
    amrex::Print() << "Done!" << std::endl;
    amrex::Print() << "Redefining mf2..." << std::endl;
    for (int lev = 0; lev < Nlev; ++lev) {
      const amrex::BoxArray ba = mf2[lev].boxArray();
      const amrex::DistributionMapping dm = mf2[lev].DistributionMap();
      constexpr int nGrow = 0;
      mf2[lev].define(ba,dm,ncomp_filter,nGrow);
    }

    amrex::Print() << "FillPatching mf1 before integration in z..." << std::endl;
    for (int lev=0; lev<Nlev; ++lev) {
      amrex::Print() << "on level "<< lev << std::endl;
      if (lev == 0) {
        amrex::PhysBCFunct<amrex::GpuBndryFuncFab<pele::physics::pltfilemanager::FillExtDirDummy>> bndry_func(
    ...
    amrex::Print() << "Integrating in z..." << std::endl;
    for (int lev=0; lev<Nlev; ++lev)
    {
      int nweights = weights[lev].size();
      int ng =  (nweights - 1)/2;
	...
	amrex::Array4<const amrex::Real> inty_arr = mf1[lev].const_array(mfi);
	amrex::Array4<amrex::Real> out_arr = mf2[lev].array(mfi);
	
	amrex::ParallelFor(bx,[=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
	  for (int n = 0; n < ncomp_filter; ++n) {
	    amrex::Real sum = 0.0;	  
	    for (int kk = -ng; kk <= ng; ++kk) {
	      sum += w[ng+kk]*inty_arr(i,j,k+kk,n);
	    }
	    out_arr(i,j,k,n) = sum;
	  }
	});
