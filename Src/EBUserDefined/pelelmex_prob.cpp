...
#if AMREX_USE_EB
   pp.get("prechamber_side_x",PeleLM::prob_parm->prechamber_side_x);
   pp.get("prechamber_side_y",PeleLM::prob_parm->prechamber_side_y);
   #if USE_DI
      //Placeholder
   #else
    pp.query("kernel_diameter",PeleLM::prob_parm->kernel_diameter);
   #endif
#else
   pp.query("jet_diameter", PeleLM::prob_parm->jet_diameter);
   pp.query("flame_height", PeleLM::prob_parm->flame_height);
   pp.query("wall_temp", PeleLM::prob_parm->wall_temp);
#endif
...
ProbParm local_prob_parm;
  ...
  amrex::Vector<amrex::Real> data(
    nx * ny * nz * 6); /* this needs to be double */ // Because 6 Values (x,y,z,u,v,w)
  if (binfmt != 0) {
  ...
  for (long i = 0; i < xinput.size(); i++) {
    xinput[i] = data[0 + i * 6];  // Every 6th value is x
    uinput[i] =
      data[3 + i * 6] * local_prob_parm.urms0 / local_prob_parm.uin_norm; //  uin_norm = 1.0 
    vinput[i] =
  ...
  local_prob_parm.Linput = xarray[nx - 1] + 0.5 * xdiff[nx - 1]; // Highest x value 
