...
static void
read_fields(const H5File& file, const std::string& mesh,
            const Vector<std::string>& vars, MultiFab& data)
{
    ...
    for (int n = 0; n < (int)vars.size(); ++n) {
        ...
        for (MFIter mfi(data); mfi.isValid(); ++mfi) {
            ...
            if (is_cv) {
                ...
                hsize_t off[4] = {(hsize_t)si, (hsize_t)lo2, (hsize_t)lo[1], (hsize_t)lo[0]};
                fsp.selectHyperslab(H5S_SELECT_SET, cnt, off);
            ...
            amrex::ParallelFor(box,
                [arr, &buf, n, lo0, lo1, lo2, nxb, nyb]
                AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
                    arr(i, j, k, n) = buf[(k-lo2)*nyb*nxb + (j-lo1)*nxb + (i-lo0)];
                });
        }
    }
}
